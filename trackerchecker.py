...
if "gistration is currently disabl" in urll:
    ...
    print(f"dream-torrents is open! {r.url}")
D_Torrents = "http://www.3dtorrents.org/index.php?page=signup"
r = requests.get(D_Torrents)
urll = r.text
if ", but registrations are closed" in urll:
    print("3D Torrents is not open")
else:
    print(f"3D Torrents is open! {r.url}")
ExoticaZ = "https://exoticaz.to/register"
r = requests.get(ExoticaZ)
urll = r.text
if "ExoticaZ is Invite On" in urll:
